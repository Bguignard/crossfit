...
class PrivateUserProfileApiTest extends AbstractIntegrationTest
{
    ...
    public function testAutoGeneratedAnalysisDoesNotBlockStandaloneAnalysisCooldown(): void
    {
        [$token, $user] = $this->createAuthenticatedUser('programming-auto-analysis-cooldown@example.com', 'programming-auto-analysis-cooldown-token');
        $performanceProfile = new UserPerformanceProfile($user);
        (new UserPerformanceMetric($performanceProfile, PerformanceMetricKeyEnum::BACK_SQUAT_1RM))->setNumericValue(150);
        ...
        self::assertResponseStatusCodeSame(201);
        self::assertSame('waiting_analysis', $this->jsonResponse()['programmingRequest']['status']);

        $this->jsonRequest('POST', '/api/me/performance-analysis-requests', [
            'parameters' => [
                'goal' => 'standalone analysis after programming',
            ],
        ], $token);

        self::assertResponseStatusCodeSame(201);
        self::assertSame('standalone analysis after programming', $this->jsonResponse()['analysisRequest']['parameters']['goal']);
    }
    ...
    public function testProgrammingRefreshRewritesExplicitStaleSourceAnalysisConstraint(): void
    {
        [$token, $user] = $this->createAuthenticatedUser('programming-explicit-stale-analysis@example.com', 'programming-explicit-stale-analysis-token');
        $performanceProfile = new UserPerformanceProfile($user);
        $backSquat = (new UserPerformanceMetric($performanceProfile, PerformanceMetricKeyEnum::BACK_SQUAT_1RM))->setNumericValue(150);
        ...
        $storedAnalysisRequest = $this->getRepository(PerformanceAnalysisRequest::class)->find($analysisPayload['id']);
        self::assertNotNull($storedAnalysisRequest);
        $storedAnalysisRequest->markCompleted(['summary' => 'Old baseline analysis.']);
        $backSquat->setNumericValue(170);
        $this->getEntityManager()->flush();
        ...
        self::assertSame('generated', $programmingPayload['analysisDependency']['mode']);
        self::assertNotSame($analysisPayload['id'], $programmingPayload['sourceAnalysisRequest']['id']);
        self::assertSame($programmingPayload['sourceAnalysisRequest']['id'], $programmingPayload['constraints']['sourceAnalysisRequestId']);
        self::assertSame($programmingPayload['sourceAnalysisRequest']['id'], $programmingPayload['analysisDependency']['source_analysis_request_id']);
    }

    public function testProgrammingReusesPendingCompatibleAnalysisRefresh(): void
    {
        [$token, $user] = $this->createAuthenticatedUser('programming-pending-analysis@example.com', 'programming-pending-analysis-token');
        $performanceProfile = new UserPerformanceProfile($user);
        (new UserPerformanceMetric($performanceProfile, PerformanceMetricKeyEnum::BACK_SQUAT_1RM))->setNumericValue(150);
        ...
        self::assertResponseStatusCodeSame(201);
        $firstPayload = $this->jsonResponse()['programmingRequest'];
        $messagesAfterFirstRequest = $this->messengerMessageCount();
        ...
        self::assertResponseStatusCodeSame(201);
        $secondPayload = $this->jsonResponse()['programmingRequest'];
        self::assertSame('waiting_analysis', $secondPayload['status']);
        self::assertSame('reused', $secondPayload['analysisDependency']['mode']);
        self::assertSame($firstPayload['sourceAnalysisRequest']['id'], $secondPayload['sourceAnalysisRequest']['id']);
        self::assertSame($messagesAfterFirstRequest, $this->messengerMessageCount());
    }
